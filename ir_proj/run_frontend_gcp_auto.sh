...
gcloud compute instances create $INSTANCE_NAME --zone=$ZONE --machine-type=n2d-standard-4 --network-interface=address=$INSTANCE_IP,network-tier=PREMIUM,subnet=default --metadata-from-file startup-script=startup_script_gcp.sh --scopes=https://www.googleapis.com/auth/cloud-platform --tags=http-server
